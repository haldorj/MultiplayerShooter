	...
	if (MuzzleFlashSocket && World)
	{
		...
		if (bUseServerSideRewind)
		{
			if (InstigatorPawn->HasAuthority()) // Server.
			{
				...
				else // Server, not locally controlled - spawn non-replicated projectile, no Server Side Rewind.
				{
			...
			{
				if (InstigatorPawn->IsLocallyControlled()) // Client, locally controlled - Spawn non-replicated projectile, Use Server Side Rewind.
				{
					SpawnedProjectile = World->SpawnActor<AProjectile>(ServerSideRewindProjectileClass, SocketTransform.GetLocation(), TargetRotation, SpawnParams);
